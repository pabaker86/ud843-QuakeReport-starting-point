...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            //Create the root level of the JSON response
            JSONObject jsonRootObject = new JSONObject(SAMPLE_JSON_RESPONSE);

            //Create the Array level of the JSON response post root level
            JSONArray jsonArray =  jsonRootObject.getJSONArray("features");

            //Loop through the array to get the items out of the properties
            for (int i=0;  i < jsonArray.length(); i++) {
                ...
                //Extract place value of json data
                String place = properties.getString("place");
                //Extract time value of json data
